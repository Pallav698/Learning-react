...
const ResturantMenu =  () => {
    ...
    return (
        <div className="top-level menu mx-auto w-6/12 my-5">
                ...
                <div className ="res-details">
                    <div>
                        <div className="mb-[18px]">
                            ...
                            <div>
                                <p className="text-[#282c3f] font-semibold mb-2 text-base">{name}</p>
                                <p className="text-[#7e808c] text-sm">{cuisines.join(" , ")}</p>
                                <p className="text-[#7e808c] text-sm">{areaName}, {sla.lastMileTravelString}</p>
                            </div>
                            ...
                            <FontAwesomeIcon icon={faStar} />{resInfo.data.cards[0].card.card.info.avgRatingString}
                                </p>
                            <p className="tot-rating text-[11px] text-[#8b8d97] font-[600]">{resInfo.data.cards[0].card.card.info.totalRatingsString}</p>
                        </button>
                            </div>
                        </div>
                        <ul className="list-none border-b-[1px] border-dashed border-[#d3d3d3]">
                            <li className="flex items-start mb-[18px]"><img src={MESS_ICON + icon } className="mr-[8px] mt-[5px]"></img>
            ...
            <ResturantDishes data= {categories}></ResturantDishes>
        </div>
    );
};
